...
class AccountInputSchema(BaseModel):
    ...
    has_profile_pic: int = Field(1, ge=0, le=1, description="1 if avatar present, 0 otherwise")
    is_verified: int = Field(0, ge=0, le=1, description="1 if verified badge, 0 otherwise")
    account_age_days: float = Field(30.0, ge=0, description="Age of account in days")
    follower_count: float = Field(100.0, ge=0, description="Number of followers")
    following_count: float = Field(150.0, ge=1, description="Number of accounts followed")
    posts_count: float = Field(25.0, ge=0, description="Total number of posts")
    ...
    posting_frequency_per_day: float = Field(1.0, ge=0, description="Average posts published per day")
    avg_engagement_rate: float = Field(3.5, ge=0, description="Average engagement percentage (%)")
    avg_likes_per_post: Optional[float] = Field(10.0, ge=0)
    avg_retweets_or_shares: Optional[float] = Field(2.0, ge=0)
    like_to_share_ratio: Optional[float] = Field(5.0, ge=0)
    mention_count_avg: Optional[float] = Field(0.5, ge=0)
